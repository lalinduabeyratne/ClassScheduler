...
import { exportStudentComprehensiveReport, exportStudentProfileSnapshotPdf } from "@/lib/billing/exportPdf";
...
export default function AdminStudentsPage() {
  ...
  function exportStudentProfileSnapshot() {
    if (!selectedStudent) {
      setActionError("Select a student before exporting a report.");
      return;
    }
    try {
      exportStudentProfileSnapshotPdf({
        student: selectedStudent,
        balance: selectedBalance,
        upcomingPrepaid: selectedUpcomingPrepaid,
        advanceBalanceCents: selectedPaymentCoverage.remainingCreditCents,
        missedSummary: selectedMissedSummary,
        attendance: selectedAttendanceView.map((session) => ({
          date: new Date(session.startAt).toLocaleString("en-LK"),
          status: session.status,
          coverup:
            session.coverupStatus === "scheduled"
              ? session.coverupSessionId && selectedSessionsById.has(session.coverupSessionId)
                ? "Scheduled"
                : "-"
              : session.coverupStatus === "completed"
                ? "Covered"
                : "-",
          coverage:
            selectedSessionsPrepaidCoverage.has(session.id) && Math.max(0, Number(session.feePerSessionCents ?? 0)) > 0
              ? "Already paid"
              : "-",
          chargeCents: session.chargeCents ?? 0,
        })),
        payments: selectedPayments.map((payment) => ({
          date: new Date(payment.paidAt).toLocaleDateString("en-LK"),
          type: (payment.paymentType ?? "single").replaceAll("_", " "),
          coverage: payment.coverageNote || "-",
          method: payment.method ? payment.method.replaceAll("_", " ") : "-",
          status: payment.status,
          slip: payment.slipPath || payment.slipUrl ? "Available" : "-",
          amountCents: payment.amountCents,
        })),
      });
      setActionSuccess(`Profile snapshot for ${selectedStudent.fullName} exported successfully.`);
    } catch (err) {
      setActionError(err instanceof Error ? err.message : "Failed to export profile snapshot.");
    }
  }
              ...
              <div className="space-y-2 rounded-lg border border-[rgb(var(--border))] p-4">
                ...
                <div className="flex flex-wrap items-end justify-between gap-3">
                  ...
                  <div className="flex w-full flex-wrap gap-2 sm:w-auto">
                    ...
                    <button className="btn btn-primary w-full sm:w-auto" onClick={exportStudentProfileSnapshot} disabled={saving}>
