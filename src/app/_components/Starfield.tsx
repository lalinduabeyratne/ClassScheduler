...
export default function Starfield({
  ...
  const mouseHandler = (event: MouseEvent) => {
    const el = canvasRef.current?.parentElement;
    if (el) {
      cursor.current.x = (event.pageX || event.clientX + el.scrollLeft - el.clientLeft) as number;
      cursor.current.y = (event.pageY || event.clientY + el.scrollTop - el.clientTop) as number;
    }
  };
  ...
  useEffect(() => {
    ...
    if (tiltAdjust) {
      window.addEventListener("deviceorientation", tiltHandler as EventListener);
    }
    if (clickToWarp) {
    ...
    return () => {
      ...
      if (tiltAdjust) {
        window.removeEventListener("deviceorientation", tiltHandler as EventListener);
      }
      if (clickToWarp) {
  ...
  }, [mouseAdjust, tiltAdjust, clickToWarp, easing, speed, warpFactor, opacity, quantity]);
